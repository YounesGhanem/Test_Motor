  ...
  else
  {
    ...
    if (pHandle->hRemainingTicks > 0U)
    {
      ...
      if (0U == pHandle->hRemainingTicks)
      {
        ...
        //HALL_SetMecAngle(HALL_Handle_t *pHandle, int16_t hMecAngle);
...
/**
  ...
  */
__weak bool HAC_GetRestartState(HallAlign_Handle_t *pHandle) //cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_ENC_ALI_CTRL
